...
- **Atlassian** (Apollo export, 2026-08-31). In-house team = "Content Studio." **PRIMARY: Sarah Fink — Executive Producer, Content Studio** (Portland OR) — exact Tier 1 target, owns production + books talent. **SECONDARY: Genna Osborne — Creative Producer, Brand** (Eugene OR) — hands-on brand-content producer, less gatekept. Alternate: **Cat Sweetland — Executive Creative Producer** (senior, poss. Sarah's peer/above). Verify Sarah + Genna on LinkedIn (current title + Content Studio = video team) before emailing; Gmail-hover emails (Google Workspace). Skip: Web Producers, Email Producers, Alicia Washington (Workplace Experience = internal events). **Lena Brooks = "Motion Producer" per Apollo, NOT the "Senior Video & Creative Producer" web search claimed — dropped.**
- **Asana** (Apollo + LinkedIn verified, 2026-08-31). No "video" titles — video runs through creative producers. **PRIMARY: Aubrey Rogers — Head of Creative Production & Operations** (SF, 5 yrs at Asana, ex-Sr Integrated & Digital Producer). ✅ LinkedIn-confirmed: "manages Marketing budget for all creative + vendor accounts for full Brand team" — she owns the purse and the vendor list, can onboard Spencer directly. Producer background = gets the craft. AI flag is soft (ops automation, not creative replacement) — keep AI topic off the table anyway. **Pitch her as a low-friction vendor add** (locked monthly rate, 24h turnaround), not "your videos need a voice." **SECONDARY: Kyle O. — Sr. Creative Producer** (check LinkedIn for video work). **Beryl Baker = SKIP** (LinkedIn-checked: events + print + physical collateral only, no video). If Kyle is also events/print, Aubrey is the sole Asana contact — fine, she owns the vendor list. Alternate: Elaine C. — Creative Director. Skip: Maddy L. ("Genius Wrangler"), Tara V. (Head of Creative Operations).
- **Monday.com, Zoom, Airtable, ClickUp, Grammarly — no contacts found.** Confirmed 2026-08-31 via both the WebSearch tool AND the in-app browser (DuckDuckGo + Bing, several phrasings, `site:linkedin.com/in`). All came up empty — this is a structural gap, not a tool choice: search engines can only surface a LinkedIn page whose indexed text happens to contain both the title and company, which is rare for non-public-facing roles. Only LinkedIn's own People-tab search (login-gated, not crawlable) can actually answer "who has title X at company Y." **Spencer's own LinkedIn browsing is the only method that works here** (proven on Jamal/Bridget/Jack/Lydia).
