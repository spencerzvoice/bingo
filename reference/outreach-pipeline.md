...
0. **An inbox reply that never gets logged into the tracker is a duplicate-outreach time bomb.** Rebeccah Sheridan (CGI/Dell) replied warmly to Spencer's Aug 4 reconnect the same day, but the reply was never entered into the tracker (no Status/Last-Contact update at the time). 24 days later, the Aug 28 batch-of-10 reconnect cross-check ("was ~18, trimmed to 10 after checking the live tracker") only checked the tracker's own rows — which still showed her as unreached — so she got a second cold reconnect email that ignored her actual reply. Caught + corrected 08/29 (row updated, apology sent in the original thread). **Lesson: before batch-sending any "reconnect"/"not yet contacted" list, cross-check against Gmail directly (not just the tracker), since the tracker can lag behind real inbox activity.**
...
**⚠️ Rebeccah Sheridan (CGI) was a duplicate, not a genuine reconnect** — she'd already replied warmly to an Aug 4 email that never made it into the tracker. Caught + corrected 08/29 (see hazard #0 above). Checked the other 9 in this batch against Gmail on 08/29: none of the others had a prior unlogged reply — Melissa Gillis, Erin Daughenbaugh, Mel Kane, Lucas Bertoli, Taylor Ballam, Mackenzie Prokos, Devin Leisher are single genuine sends awaiting reply; Evan Kay and Evan Romoff already replied to *this* Aug 28 send (not a duplicate — normal same-day response). Tracker is clean as of 08/29.
